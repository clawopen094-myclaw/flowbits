...
export const OpenAiIcon = ({
  ...
  const { resolvedTheme } = useTheme(); // Get theme inside the component
  const [iColor, setiColor] = useState(actualTheme|| resolvedTheme); // Initialize state with resolvedTheme or fallback to theme
  
  useEffect(() => {
      setiColor(resolvedTheme); // Update iColor when theme changes
  }, [resolvedTheme]);
  
  const color = iColor === 'dark' ? '#fff' : '#000'; // Set color based on theme
    ...
    >
      <defs>
        <radialGradient
          ...
          gradientTransform="matrix(516.16 174.57 -1398.25 4135.43 50.52 208.1)"
        >
          <stop offset=".067" stopColor={color} />
          <stop offset=".343" stopColor={color} />
          <stop offset=".672" stopColor={color} />
        </radialGradient>
      </defs>
      <path
